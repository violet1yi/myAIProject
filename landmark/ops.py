...
#dense(inputs, units, activation=None,use_bias=True,kernel_initializer=None,bias_initializer=tf.zeros_initializer(),kernel_regularizer=None,
    # bias_regularizer=None,activity_regularizer=None, trainable=True, name=None, reuse=None)
#reference alexnet
'''
...
return out
'''
def fc1(inputs, units, activation, keep_prop = 0.7, use_bias = True, name = 'fc1'):
    input_shape = inputs.get_shape()
    dense1 = tf.layers.dense(inputs, units, activation, use_bias)
    dense_drop = tf.nn.dropout(dense1, keep_prop)
    #dense_drop = tf.reshape(dense_drop, shape=[None, 10])
    return dense_drop

def fc2(inputs, units, activation, use_bias = True, name = 'fc2'):
    dense2 = tf.layers.dense(inputs, units, activation, use_bias)
    dense2 = tf.reshape(dense2, shape=[-1, 10])
    return dense2