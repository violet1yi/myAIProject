...
def main():
    ...
    # read queue
    glob_pattern = os.path.join(args.dataset_dir, 'train.tfrecord')
    print('path', glob_pattern)
    img_batch, label_batch = createBatch(glob_pattern, 50)
    #print('size of image batch: %s, label batch: %s'%(type(img_batch), type(label_batch)))

    coord = tf.train.Coordinator()
    #threads = tf.train.start_queue_runners(sess=sess, coord=coord)

    #inputs_img = tf.placeholder(tf.float32, [None, height, width, 3], name='input1')
    #inputs_labels = tf.placeholder(tf.float32, [None, 10], name='input2')

    net_out = mobilenetv2(img_batch, is_train=args.is_train)
    # loss
    '''
    ...
    '''
    euclidean_loss = tf.reduce_mean(tf.reduce_sum(tf.square(tf.subtract(tf.cast(label_batch, tf.float32), net_out)), axis=1)/2, axis=0)
    #l2_loss = tf.add_n(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
    ...
    # summary
    tf.summary.scalar('total_loss', euclidean_loss)
    ...
    for _step in range(step+1, max_steps+1):
        ...
        #feed_dict = {global_step: _step, inputs_img: img_batch, inputs_labels: label_batch}
        ...
        if _step % 10 == 0:
            _summ, _loss = sess.run([summary_op, euclidean_loss],  feed_dict=feed_dict)
            writer.add_summary(_summ, _step)
            print('global_step:{0}, time:{1:.3f}, lr:{2:.8f}, loss:{4:.6f}'.format
                  (_step, time.time() - start_time, _lr, _loss))
